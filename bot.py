...
# ════════════════════════════════════════════════════════════
#  TELEGRAM-SAFE BUTTON THEME
# ════════════════════════════════════════════════════════════
# Telegram clients own the native button surface. The Bot API does not
# accept CSS, background colors, gradients, border radius, hover/pressed
# states, or a disabled attribute for inline/reply keyboards. Sending
# unsupported styling fields would make the keyboard invalid.
#
# The theme below therefore uses a consistent, accessible color-marker
# system. It works in both Telegram light and dark mode, keeps callback
# payloads unchanged, and gives every button an explicit visual role.
BUTTON_THEME = {
    "primary":   {"marker": "🔷", "label": "Primary"},
    "success":   {"marker": "🟢", "label": "Success"},
    "warning":   {"marker": "🟠", "label": "Warning"},
    "danger":    {"marker": "🔴", "label": "Danger"},
    "secondary": {"marker": "⚫", "label": "Secondary"},
    "active":    {"marker": "✨", "label": "Selected"},
    "disabled":  {"marker": "⚪", "label": "Unavailable"},
}

_TELEGRAM_BUTTON = Button


def _button_tone(text: str, payload: str = "") -> str:
    """Infer a stable semantic tone without changing button behavior."""
    value = f"{text} {payload}".lower()
    if payload in {
        "btn_get_number", "btn_countries", "btn_support", "btn_refer",
        "view_user", "go_admin", "select_svc", "adm_countries",
        "adm_panels", "adm_code", "adm_ai",
    } or payload.startswith(("svc_", "ctry_", "up_", "upc_")):
        return "primary"
    if payload in {"btn_active", "code_dl", "ai_chat", "addadm", "add_panel"}:
        return "success"
    if payload == "adm_dummy" or any(
        word in value for word in (
            "cancel", "delete", "remove", "stop", "❌", "বাতিল", "মুছুন",
        )
    ):
        return "disabled" if payload == "adm_dummy" else "danger"
    if any(
        word in value for word in (
            "broadcast", "set ", "setting", "edit", "upload", "add ",
            "add_", "restart", "quota", "range", "toggle", "change",
            "সেট", "সেটিং", "এডিট", "আপলোড", "যোগ", "রিস্টার্ট", "কোটা",
            "চেঞ্জ", "পরিবর্তন",
        )
    ):
        return "warning"
    if any(
        word in value for word in (
            "save", "activate", "active", "confirm", "copy", "get number",
            "নাম্বার নিন", "যোগ",
        )
    ):
        return "success"
    if any(
        word in value for word in (
            "support", "share", "community", "open", "download", "ask",
            "ai ", "service", "country", "otp group", "home", "হোমে",
        )
    ):
        return "primary"
    return "secondary"
...
def _decorate_button_text(
    text: str,
    tone: str = "",
    active: bool = False,
    disabled: bool = False,
) -> str:
    """Add an always-visible semantic marker to a Telegram button label."""
    tone = "disabled" if disabled else (tone or _button_tone(text))
    if active:
        tone = "active"
    if tone not in BUTTON_THEME:
        tone = "secondary"
    marker = BUTTON_THEME[tone]["marker"]
...
def _bot_button(
    ...
    *,
    ...
    """Build a valid Bot API button with themed, high-contrast text."""
    button = {
        "text": _decorate_button_text(text, tone, active, disabled),
    }
...
class _ThemedButton:
    """Drop-in Telethon Button facade used by every existing keyboard."""

    @staticmethod
    def inline(text, data, *, tone="", active=False, disabled=False):
        return _TELEGRAM_BUTTON.inline(
            _decorate_button_text(text, tone, active, disabled), data
        )

    @staticmethod
    def url(text, url, *, tone="", active=False, disabled=False):
        return _TELEGRAM_BUTTON.url(
            _decorate_button_text(text, tone, active, disabled), url
        )

    @staticmethod
    def text(text, **kwargs):
        """Themed reply/menu button for any future reply keyboard."""
        return _TELEGRAM_BUTTON.text(_decorate_button_text(text), **kwargs)


# Existing Button.inline/Button.url calls now share the theme automatically.
Button = _ThemedButton
...
BOT_TOKEN   = '8667327104:AAE4SB0_aUZnk5AxTf3SKCSVmyruSXF4zTs'
...
def otp_hook():
    try:
        ...
        group_kbd = {"inline_keyboard": [[
            _bot_button("🚀 Community", url=comm_link, tone="primary"),
            _bot_button("🗄 Number", url=f"https://t.me/{BOT_TOKEN.split(':')[0]}",
                        tone="secondary"),
        ]]}
...
CREATE TABLE IF NOT EXISTS api_ranges(
    country TEXT, service TEXT, rid TEXT, PRIMARY KEY(country, service));
''')
...
c.execute("INSERT OR IGNORE INTO bot_settings VALUES ('stex_base_url','')")
c.execute("INSERT OR IGNORE INTO bot_settings VALUES ('stex_api_key','')")
if SUPER_ADMIN:
...
SYSTEM_PROMPT = (
    ...
    "Features: live STEX SMS API number allocation with local-stock fallback, 100+ countries, "
    "admin panel, SMS panels, multi-admin (5), bn/en language, OTP webhook, AI assistant, code editor. "
...
KB = {
    ...
    'api':       "🌐 STEX API:\nAdmin → 🌐 API Settings → Base URL + API Key → ➕ Add Range (country service rid)\n✅ Bot tries the live API first, falls back to local stock if it can't allocate.",
}
...
# ════════════════════════════════════════════════════════════
#  STEX SMS API  (getnum / liveaccess / success-otp / console)
# ════════════════════════════════════════════════════════════
def stex_cfg():
    return gset('stex_base_url', '').strip().rstrip('/'), gset('stex_api_key', '').strip()

def stex_call(method, path, payload=None, timeout=15):
    base, key = stex_cfg()
    if not base or not key: return None
    try:
        url = f"{base}{path}"
        headers = {"mauthapi": key, "content-type": "application/json"}
        if method == "POST":
            r = http_requests.post(url, headers=headers, json=payload or {}, timeout=timeout)
        else:
            r = http_requests.get(url, headers=headers, timeout=timeout)
        return r.json()
    except Exception as e:
        print(f"STEX API error [{path}]: {e}")
        return None

def stex_getnum(rid):
    """Allocate one number from a range via the live API. Returns full_number or None."""
    j = stex_call("POST", "/getnum", {"rid": rid})
    if j and j.get('meta', {}).get('code') == 200:
        full = (j.get('data') or {}).get('full_number')
        if full: return full
    return None

def stex_liveaccess():
    j = stex_call("GET", "/liveaccess")
    if j and j.get('meta', {}).get('code') == 200:
        return (j.get('data') or {}).get('services', [])
    return []

def stex_success_otp():
    j = stex_call("GET", "/success-otp")
    if j and j.get('meta', {}).get('code') == 200:
        return (j.get('data') or {}).get('otps', [])
    return []

def stex_console():
    j = stex_call("GET", "/console")
    if j and j.get('meta', {}).get('code') == 200:
        return (j.get('data') or {}).get('hits', [])
    return []

def get_api_rid(country, service):
    c.execute("SELECT rid FROM api_ranges WHERE country=? AND service=?", (country, service))
    r = c.fetchone(); return r[0] if r else None

def try_api_number(country, service):
    """Attempt to allocate a fresh number from the live API for this country+service.
    Returns a formatted number string, or None if unavailable (caller should fall back)."""
    base, key = stex_cfg()
    if not base or not key: return None
    rid = get_api_rid(country, service)
    if not rid: return None
    full = stex_getnum(rid)
    return fmt_num(full) if full else None
...
def send_compact_kb(chat_id: int, text: str, lang: str = 'en', parse_mode: str = 'Markdown'):
    http_requests.post(f"{_TGAPI}/sendMessage", json={
        ...
        "reply_markup": {
            "inline_keyboard": [
                [_bot_button("📱 নাম্বার নিন", callback_data="btn_get_number",
                             tone="primary"),
                 _bot_button("🌍 দেশ দেখুন", callback_data="btn_countries",
                             tone="primary")],
                [_bot_button("📋 আমার নাম্বার", callback_data="btn_active",
                             tone="success"),
                 _bot_button("🔍 নাম্বার খুঁজুন", callback_data="btn_search",
                             tone="secondary")],
                [_bot_button("💬 সাপোর্ট", callback_data="btn_support",
                             tone="primary"),
                 _bot_button("👥 রেফার করুন", callback_data="btn_refer",
                             tone="success")],
                [_bot_button("📊 আমার স্ট্যাটাস", callback_data="btn_status",
                             tone="secondary"),
                 _bot_button("🌐 ভাষা পরিবর্তন", callback_data="btn_lang",
                             tone="warning")],
            ]
        }
    }, timeout=10)
...
def adm_kb():
    return [
        ...
        [Button.inline("🌐 API সেটিংস","adm_api")],
...
async def show_numbers(event, uid, svc, short, edit=True, reset_shown=False):
    ...
    api_num = try_api_number(short, svc)
    if api_num:
        try:
            c.execute("INSERT OR IGNORE INTO premium_stock(country,service,number,status)VALUES(?,?,?,0)",
                      (short, svc, api_num)); db.commit()
    ...
    if not rows:
        kbd = [[_bot_button("🌍 Change Country",
                            callback_data=f"svc_{svc}", tone="primary")],
               [_bot_button("◀️ Back to Services",
                            callback_data="select_svc", tone="secondary")]]
    ...
    for _db_id, num in rows:
        nf = fmt_num(num)
        kbd.append([_bot_button(f"{c_flag} 📋  {nf}", copy_text=nf,
                                tone="success")])

    kbd += [
        [_bot_button("🔄 Change Number",
                     callback_data=f"chg_{svc}_{short}", tone="warning"),
         _bot_button("🌍 Change Country",
                     callback_data=f"svc_{svc}", tone="primary")],
        [_bot_button("📢 OTP Group ↗", url=glink("otp_group"), tone="primary")],
    ]
    if is_admin(uid):
        kbd.append([_bot_button("🛠 Admin Panel",
                                callback_data="go_admin", tone="secondary")])
...
async def on_msg(event):
    ...
    if state == "stex_base":
        c.execute("INSERT OR REPLACE INTO bot_settings VALUES('stex_base_url',?)",
                  (text.strip().rstrip('/'),)); db.commit()
        await event.respond("✅ API Base URL saved.", buttons=[[Button.inline("🔙","adm_api")]]); return

    if state == "stex_key":
        c.execute("INSERT OR REPLACE INTO bot_settings VALUES('stex_api_key',?)",
                  (text.strip(),)); db.commit()
        await event.respond("✅ API Key saved.", buttons=[[Button.inline("🔙","adm_api")]]); return

    if state == "api_range_add":
        parts = text.strip().split()
        if len(parts) != 3:
            await event.respond(
                "❌ Format: `country_code service rid`\nExample: `uk whatsapp 26134`",
                buttons=[[Button.inline("🔙","adm_ranges")]], parse_mode='md'); return
        cshort, svc_k, rid = parts[0].lower(), parts[1].lower(), parts[2]
        c.execute("INSERT OR REPLACE INTO api_ranges(country,service,rid) VALUES(?,?,?)",
                  (cshort, svc_k, rid)); db.commit()
        cn, cf = ctry(cshort)
        await event.respond(f"✅ Range mapped: {cf} {cn} — {SVC.get(svc_k, svc_k)} → `{rid}`",
                            buttons=[[Button.inline("🔙","adm_ranges")]], parse_mode='md'); return
...
async def on_cb(event):
    ...
    if data == "adm_api":
        base, key = stex_cfg()
        masked_key = (key[:6] + "…" + key[-4:]) if len(key) > 12 else ("(set)" if key else "❌ Not set")
        txt = (
            f"🌐 **STEX API Settings**\n\n"
            f"🔗 Base URL: `{base or '❌ Not set'}`\n"
            f"🔑 API Key: `{masked_key}`\n\n"
            f"Bot tries the live API first for a number; if it can't allocate one, "
            f"it falls back to local uploaded stock."
        )
        await event.edit(txt, buttons=[
            [Button.inline("✏️ Set Base URL","set_stex_base"), Button.inline("🔑 Set API Key","set_stex_key")],
            [Button.inline("🔢 Range Mapping","adm_ranges")],
            [Button.inline("🔙","adm_back")],
        ], parse_mode='md'); return

    if data == "set_stex_base":
        STATES[uid] = "stex_base"
        await event.edit(
            "✏️ Send the API Base Path (from the docs' BASE PATH field), e.g.\n"
            "`https://api.2oo9.cloud/XXXXXXXXXX/xxxxx/@public/api`",
            buttons=[[Button.inline("🔙","adm_api")]], parse_mode='md'); return

    if data == "set_stex_key":
        STATES[uid] = "stex_key"
        await event.edit("🔑 Send your API Key (the `mauthapi` token, from your Profile page):",
                         buttons=[[Button.inline("🔙","adm_api")]]); return

    if data == "adm_ranges":
        c.execute("SELECT country,service,rid FROM api_ranges ORDER BY country,service")
        rows = c.fetchall()
        txt = "🔢 **Range Mapping** (country + service → API rid)\n\n"
        btns = []
        for cshort, svc_k, rid in rows:
            cn, cf = ctry(cshort)
            txt += f"{cf} {cn} — {SVC.get(svc_k, svc_k)} → `{rid}`\n"
            btns.append([Button.inline(f"❌ {cf} {cn} / {SVC.get(svc_k, svc_k)}", f"rmrange_{cshort}_{svc_k}")])
        if not rows: txt += "❌ None mapped yet."
        btns.append([Button.inline("➕ Add Range","add_range")])
        btns.append([Button.inline("🔙","adm_api")])
        await event.edit(txt, buttons=btns, parse_mode='md'); return

    if data == "add_range":
        STATES[uid] = "api_range_add"
        await event.edit(
            "➕ **Add Range**\n\nSend: `country_code service rid`\n"
            "Example: `uk whatsapp 26134`\n\n"
            "(country_code = short code used in this bot, e.g. uk, us, bd)",
            buttons=[[Button.inline("🔙","adm_ranges")]], parse_mode='md'); return

    if data.startswith("rmrange_"):
        rest = data[8:]; cshort, svc_k = rest.split("_", 1)
        c.execute("DELETE FROM api_ranges WHERE country=? AND service=?", (cshort, svc_k)); db.commit()
        await event.answer("✅ Removed!", alert=False)
        c.execute("SELECT country,service,rid FROM api_ranges ORDER BY country,service")
        rows = c.fetchall()
        txt = "🔢 **Range Mapping** (country + service → API rid)\n\n"
        btns = []
        for cshort2, svc_k2, rid2 in rows:
            cn, cf = ctry(cshort2)
            txt += f"{cf} {cn} — {SVC.get(svc_k2, svc_k2)} → `{rid2}`\n"
            btns.append([Button.inline(f"❌ {cf} {cn} / {SVC.get(svc_k2, svc_k2)}", f"rmrange_{cshort2}_{svc_k2}")])
        if not rows: txt += "❌ None mapped yet."
        btns.append([Button.inline("➕ Add Range","add_range")])
        btns.append([Button.inline("🔙","adm_api")])
        await event.edit(txt, buttons=btns, parse_mode='md'); return
    ...
    if data == "adm_code":
        ...
        await event.edit(
            ...
            f"Features: STEX Live API + Local Stock • 5 Services • 100+ Countries\n"
